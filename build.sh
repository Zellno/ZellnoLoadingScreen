...
SOURCE_DIR="$PROJECT_DIR/source/ZellnoLoadingScreen"
BUILD_ROOT="$PROJECT_DIR/build"
BUILD_MOD="$BUILD_ROOT/@ZellnoLoadingScreen"
...
EXPECTED_BUILD_MOD="$PROJECT_DIR/build/@ZellnoLoadingScreen"
...
for required in \
    ...
    "$SOURCE_DIR/loadingscreen.edds" \
    "$SOURCE_DIR/Scripts/3_Game/ZellnoLoadingScreen/ZellnoLoadingScreen.c" \
    "$PROJECT_DIR/mod.cpp" \
...
wine "$FILEBANK" \
    -property prefix=ZellnoLoadingScreen \
    -dst "$(winepath -w "$BUILD_ADDONS")" \
    "$(winepath -w "$SOURCE_DIR")"

PBO="$BUILD_ADDONS/ZellnoLoadingScreen.pbo"
